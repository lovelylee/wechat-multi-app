...
const execAsync = require$$5.promisify(exec);
class WeChatManager {
  // Sudo exec wrapper
  static async sudoExec(command) {
    return new Promise((resolve, reject) => {
      sudoPrompt.exec(command, { name: this.APP_NAME }, (error, stdout) => {
        if (error) {
          reject(error);
        } else {
  ...
  // Create instances
  static async createInstances(baseName, count) {
    const cmds = [];
  ...
  // Launch instance
  static async launchInstance(appPath) {
    await execAsync(`open -n "${appPath}"`);
  }
  // Delete instance
  static async deleteInstance(appPath) {
  ...
  static async getRunningInstances() {
    try {
  ...
  static async stopInstance(appPath) {
    try {
  ...
  // Rename instance
  static async renameInstance(oldPath, newName) {
...
__publicField(WeChatManager, "WECHAT_PATH", "/Applications/WeChat.app");
__publicField(WeChatManager, "APP_DIR", "/Applications");
__publicField(WeChatManager, "APP_NAME", "WeChatMultiManager");
__publicField(WeChatManager, "LSREGISTER", "/System/Library/Frameworks/CoreServices.framework/Frameworks/LaunchServices.framework/Support/lsregister");
...
function setupHandlers() {
  ...
  ipcMain.handle("wechat:create", async (_, name, count) => {
    await WeChatManager.createInstances(name, count);
    return true;
  });
  ipcMain.handle("wechat:launch", async (_, path2) => {
    await WeChatManager.launchInstance(path2);
  });
  ipcMain.handle("wechat:delete", async (_, path2) => {
    await WeChatManager.deleteInstance(path2);
  });
  ipcMain.handle("wechat:running", async () => {
    return WeChatManager.getRunningInstances();
  });
  ipcMain.handle("wechat:stop", async (_, path2) => {
    await WeChatManager.stopInstance(path2);
  });
  ipcMain.handle("wechat:rename", async (_, oldPath, newName) => {
